#!/usr/bin/env python3
"""Generate daily UPSC brief from a newAgg edition and post to Telegram.
...
def build_telegram_parts(data: dict) -> list[str]:
    ...
    p1: list[str] = []
    p1.append(f"📰 <b>newAgg Daily Brief — {_fmt_date(edition_date)}</b>")
    ...
    p2.append(health)
    p2.append("")
    p2.append("<i>Source-linked India current affairs for UPSC prep · #newAgg</i>")
...
def build_markdown(data: dict) -> str:
    ...
    lines: list[str] = []
    lines.append(f"# newAgg Daily Brief — {_fmt_date(edition_date)}")
    ...
    lines.append(f"*{health_line} · Generated by [newAgg](https://github.com/)*")
    lines.append("")
